...
{
    public class Problem_59 : Problem
    {
        //https://projecteuler.net/problem=59 Published on Friday, 19th December 2003, 06:00 pm; Solved by 36243;        Difficulty rating: 5%
        //                                      
        ...
        public override object RunProblem(double x, double y = 0, double z = 0)
        {
            for(var i = 0; i < 26; i++)
                for(var j = 0; j < 26; j++)
                    for(var k = 0; k < 26; k++)
                    {
                        var key = new List<int>() { i + 97, j + 97, k + 97 };
                        
                        var chars = new List<char>();
                        var failed = false;
                        for (var m = 0; !failed && m < Constants._e59_cipher.Count(); m++)
                        {
                            var c = (char)(Constants._e59_cipher[m] ^ key[m % 3]);
                            if (c < 32 || c > 122)
                                failed = true;
                            else
                                chars.Add(c); 
                        ...
                        var message = string.Join("", chars);
                        if (!failed && message.Contains("extract"))
                        {
                            Debug.WriteLine("-----");
                            Debug.WriteLine(message);
                            Debug.WriteLine(chars.Sum(c => c));
                            Debug.WriteLine("-----");
                        }                        
                    }
            return new { result = 0 };
